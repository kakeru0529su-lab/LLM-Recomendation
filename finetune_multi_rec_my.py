...
def train(
    ...
    # --- 使用したトレーニングデータをファイルに保存 ---
    import json
    print("Saving training and validation data to files...")
    train_data_file = os.path.join(output_dir, "used_train_data.json")
    val_data_file = os.path.join(output_dir, "used_val_data.json")
    
    # 出力ディレクトリが存在しない場合は作成
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)
    
    # 訓練データを保存（tokenize前の生データ）
    ...
    print(f"Training data saved to {train_data_file} ({len(train_data_list)} samples)")
    
    # 検証データを保存（tokenize前の生データ）
    with open(val_data_file, 'w', encoding='utf-8') as f:
        val_data_list = []
        for item in val_data["train"]:
    ...
    print(f"Validation data saved to {val_data_file} ({len(val_data_list)} samples)")
    ...
    class AUCLoggerCallback(TrainerCallback):
        def on_evaluate(self, args, state, control, metrics=None, **kwargs):
            if metrics is not None and "eval_auc" in metrics:
                auc = metrics["eval_auc"]
                log_path = os.path.join(args.output_dir, "auc_log.txt")
                with open(log_path, "a") as f:
                    f.write(f"epoch: {state.epoch}, step: {state.global_step}, auc: {auc}\n")
    ...
    trainer = transformers.Trainer(
        ...
        args=transformers.TrainingArguments(
            ...
            report_to=None,
            seed=seed,  # シード値を設定
            data_seed=seed,  # データローダーのシード値も設定
            dataloader_num_workers=4,  # ワーカー数を指定
        ...
        callbacks = [EarlyStoppingCallback(early_stopping_patience=10), AUCLoggerCallback()]
    )
