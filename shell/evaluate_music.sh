...
test_data='./data/music/test.json'
...
# music用のディレクトリを作成
result_dir="${parent_dir}/results_music_detail"
mkdir -p "$result_dir"

# resultファイル名を生成（adaptation有効時は_adaptedを付与）
if [ "$use_adaptation" = "True" ]; then
    result_file="${result_dir}/${checkpoint_name}_adapted.json"
else
...
if [ -d "$checkpoint_path" ]; then
    ...
    CUDA_VISIBLE_DEVICES=$CUDA_ID python evaluate_maml_with_samples.py \
