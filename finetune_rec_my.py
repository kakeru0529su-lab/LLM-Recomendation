...
def train(
    ...
    model = LlamaForCausalLM.from_pretrained(
        base_model,
        load_in_8bit=False,  # これをFalseに変更
        torch_dtype=torch.bfloat16, # RTX6000proはbfloatが有効
        device_map=device_map,
    )
    ...
    #     # NaNや無効な値をチェック
    #     if len(pre[0]) == 0 or len(pre[1]) == 0:
    #         return {'auc': 0.0}
        
    #     # NaNを除外
    #     valid_mask = ~(np.isnan(pre[0]) | np.isnan(pre[1]))
    #     if not valid_mask.any():
    #         return {'auc': 0.0}
        
    #     valid_predictions = pre[0][valid_mask]
    #     valid_labels = pre[1][valid_mask]
        
    #     # クラスが1つしかない場合もAUCは計算できない
    ...
    #     This is a workaround to avoid storing too many tensors that are not needed.
    #     """
    #     # 特定のトークンID（8241または3782）の位置を見つける
    #     labels_index = torch.argwhere(torch.bitwise_or(labels == 8241, labels == 3782))
        
    #     # ラベルインデックスが空の場合の処理
    #     if labels_index.shape[0] == 0:
    #         return torch.tensor([]), torch.tensor([])
        
    #     # ゴールドラベルを作成（3782 -> 0, 8241 -> 1）
    #     gold = torch.where(labels[labels_index[:, 0], labels_index[:, 1]] == 3782, 0, 1)
    ...
    #     # 範囲チェック
    #     valid_indices = labels_index[:, 1] >= 0
    #     if not valid_indices.any():
    #         return torch.tensor([]), torch.tensor([])
        
    #     labels_index = labels_index[valid_indices]
    #     gold = gold[valid_indices]
        
    #     # ソフトマックスを適用
    #     logits = logits.softmax(dim=-1)
    #     selected_logits = logits[labels_index[:, 0], labels_index[:, 1]][:, [3782, 8241]]
    #     logits_probs = torch.softmax(selected_logits, dim=-1)
        
    #     # 3つごとのサンプリング（2::3は2番目から3つおき）
    #     sampled_probs = logits_probs[:, 1][2::3]
    #     sampled_gold = gold[2::3]
        
    #     return sampled_probs, sampled_gold
    ...
    trainer = transformers.Trainer(
        ...
        args=transformers.TrainingArguments(
            ...
            # fp16=True,
            fp16=False,      # fp16を無効化
            bf16=True,       # bf16を有効化（RTX6000pro対応）
            # max_grad_norm=1.0,  # 最大値を設定
            logging_steps=8,
            optim="adamw_torch",
